...
const DESTINATION_EMAIL = 'guruvishnu1927@gmail.com';
...
export const ContactForm: React.FC = () => {
  ...
  const onSubmit = async (data: ContactFormData) => {
    setSubmitError(null);
    try {
      // 1. Direct FormSubmit AJAX Endpoint targeting guruvishnu1927@gmail.com
      ...
      console.warn('FormSubmit AJAX failed, activating direct mailto fallback:', err);
      // 2. Direct Mailto Fallback: Launch pre-filled email client to deliver to guruvishnu1927@gmail.com
      const subject = encodeURIComponent(
        `Portfolio Contact: ${data.name}${data.company ? ` from ${data.company}` : ''}`
      );
    ...
    return (
      ...
      >
        ...
        <p className="text-sm text-[#8A8A8E]">
          ...
          <strong className="text-[#F5F5F7]">{DESTINATION_EMAIL}</strong>.
        </p>
        <button
          onClick={() => setIsSubmitted(false)}
          className="text-xs font-mono text-[#4F8CFF] underline cursor-pointer hover:text-white transition-colors"
        >
